...
`mkvirtualenv inv -r requirements.txt --python=/usr/bin/python3` to create new 
virtualenvwrapper (will be atomatically activated - normally `workon inv`) with all requirements -change `/usr/bin/python3` to your python path 
...
Afterwards `deactivate` to deactivate the virtualenvwrapper